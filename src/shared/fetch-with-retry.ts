...
export function fetchBuilder<
  F extends (...args: any) => Promise<any> = typeof fetch
>(
  fetchFunc: F,
  params: FetchRetryParams = {}
): (
  ...
  return function (
    ...
    const retryDelayFn =
      typeof frp.retryDelay === "function"
        ? frp.retryDelay
        : (): number => frp.retryDelay as number;
    ...
    const retryOnFn =
      typeof frp.retryOn === "function"
        ...
        : (
            ...
            (!!error ||
              !response ||
              (frp.retryOn as number[]).indexOf(response.status) !== -1) &&
            attempt < retries;
...
const options = {
  ...
  ): number => Math.pow(2, attempt) * 500,
  retryOn: [419, 503, 504],
};

export const fetchWithRetry = fetchBuilder(fetch, options);
